...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for iteration in range(0, self.iterations):
            newQs = self.values.copy()
            allStates = self.mdp.getStates()
            for state in allStates:
                if self.mdp.isTerminal(state):
                    continue
                bestAction = self.computeActionFromValues(state)
                nextQ = self.computeQValueFromValues(state, bestAction)
                newQs[state] = nextQ
            self.values = newQs
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, prob in transAndProbs:
            nextR = self.mdp.getReward(state, action, nextState)
            nextQ = self.getValue(nextState)
            Q += nextR + self.discount * prob * nextQ
        return Q
