...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if problem.isGoalState(state):
        return 0
    distances = []
    for coordinates, visited in enumerate(state[1]):
        if  visited == 0:
            distances.append(mazeDistance(position, corners[coordinates], problem.startState))
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    distancesToFood = map(lambda xy: mazeDistance(xy, position, problem.startingGameState), foodLocations)
