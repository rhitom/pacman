...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for toElim in eliminationOrder:
            current, temp = joinFactorsByVariable(current, toElim)
            if len(temp.unconditionedVariables()) != 1:
                elim = eliminate(temp, toElim)
                current.append(elim)
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if self.particles.getBeliefDistribution().total() == 0:
        #     self.initializeUniformly(gameState)
        jailPos = self.getJailPosition()
        pacPos = gameState.getPacmanPosition()
        ...
        for location in self.particles:
            prob = self.getObservationProb(observation, pacPos, location, jailPos)
            particleDist[location] += prob
        if particleDist.total() == 0:
            ...
            # newParticles = []
            for particle in self.particles:
                particle = particleDist.sample()
                # self.particles[i] = particleDist.sample()
            # self.partciles = newParticles
        # self.beliefs.normalize()
        return
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(0, len(self.particles)):
            if self.particles[i] not in oldParticles.keys():
                ...
                oldParticles[self.particles[i]] = newPosDist
